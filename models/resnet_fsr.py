...
class Separation(torch.nn.Module):
    ...
    def forward(self, feat, is_eval=False):
        ...
        # version 3
        # fc_in = torch.mean(rob_map.view(rob_map.shape[0], rob_map.shape[1], -1), dim=-1)
        # fc_out = self.fc(fc_in.view(rob_map.shape[0], rob_map.shape[1]))
        # if not is_eval:
...
class ResNet(nn.Module):
    ...
    def forward(self, x, is_eval=False):
        ...
        # v3
        # rec_r_feat = self.recalibration(r_feat, 1 - mask)
