...
def train(epoch):
    ...
    with tqdm(total=(len(trainset) - len(trainset) % args.bs)) as _tqdm:
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            nat_outputs, _, _, _ = net(inputs, targets)
            adv_outputs, adv_r_outputs, adv_nr_outputs, adv_rec_outputs = net(adv_inputs, targets)
            # 对抗标签，获取top2标签
            ...
            # t_real = torch.ones((adv_outputs.size(0), 1))
            # t_fake = torch.zeros((adv_outputs.size(0), 1))
            # nr_rec_loss = torch.tensor(0.).to(device)
            # if not len(adv_rec_outputs) == 0 and len(adv_nr_outputs) == 0:
            #     for rec_out, nr_out in adv_rec_outputs, adv_nr_outputs:
            #         nr_rec_loss += (BCE_stable(rec_out - torch.mean(nr_out), t_real) +
            #                       BCE_stable(nr_out - torch.mean(rec_out), t_fake)) / 2
            #     nr_rec_loss /= len(adv_rec_outputs)
            #
            # # r_rec_loss
            # t_real = torch.ones((adv_outputs.size(0), 1))
            # t_fake = torch.zeros((adv_outputs.size(0), 1))
            # r_rec_loss = torch.tensor(0.).to(device)
            # if not len(adv_r_outputs) == 0 and len(adv_rec_outputs) == 0:
            #     for r_out, rec_out in adv_r_outputs, adv_rec_outputs:
            #         r_rec_loss += (BCE_stable(rec_out - torch.mean(r_out), t_real) +
            #                         BCE_stable(r_out - torch.mean(rec_out), t_fake)) / 2
            #     r_rec_loss /= len(adv_r_outputs)
            ...
            # loss = adv_cls_loss + 6 * (r_nr_loss + nr_rec_loss + r_rec_loss)
            loss = adv_cls_loss
...
def test(epoch):
    ...
    with tqdm(total=(len(testset) - len(testset) % args.bs), dynamic_ncols=True) as _tqdm:
        ...
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            ori_outputs, ori_r_outputs, ori_nr_outputs, ori_rec_outputs = net(inputs, targets, is_eval=True)
            adv_outputs, adv_r_outputs, adv_nr_outputs, adv_rec_outputs = net(adv_inputs, targets, is_eval=True)
